    ...
    Column (
        ...
        Row (
            ...
            for (dots in 1..passcode.length){
                Text("*", modifier = Modifier.padding(end = 4.dp), style = MaterialTheme.typography.headlineLarge,
            ...
            if (passcode.length < 4){
                ...
                if (it == "del"){
                    ...
                    scope.launch {
                        ...
                        scope.launch {
                            themeDataStore.savePasscode(hashed)
                            themeDataStore.saveHasPasscode(true)
        ...
        if (hasPasscode != null && hasPasscode){
            Button(
                onClick = {
                    scope.launch {
                        themeDataStore.savePasscode("")
                        themeDataStore.saveHasPasscode(false)
                        navController.popBackStack()
                    }
                }
            ) {
